...
export default function NodeBanner(props: { className?: string }) {
  ...
  // Variables to be set by methods:
  ...
  let linearGradient: CanvasGradient, linearGradientToColor: string;
  let radialGradient: CanvasGradient, radialGradientTransparency: string;
  ...
  useEffect(() => {
    ...
    theme = resolvedTheme === 'light' ? document.querySelector('.light')! : document.querySelector('.dark')!;
    ...
    linearGradientToColor = window.getComputedStyle(theme).getPropertyValue('--banner-linear-gradient-to-color');
    // Alpha of bright center circle of radial gradient:
    radialGradientTransparency = resolvedTheme === 'light' ? '1' : '0.13';

    setupCanvasData(); // Set up canvas

    // Create linear gradient for canvas background:
    linearGradient = context.createLinearGradient(0, 0, 0, height);
  ...
  function loop() {
    ...
    // (Before nodes and lines, so they appear on top of it)
    linearGradient.addColorStop(0.0, bgColor);
    linearGradient.addColorStop(0.8, bgColor); // Start color transition at 80%
    linearGradient.addColorStop(1.0, linearGradientToColor);
    ...
    radialGradient.addColorStop(0, `rgba(255, 255, 255, ${radialGradientTransparency})`); // Bright center
